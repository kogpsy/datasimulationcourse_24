...
Now we specify the random effect structure. We'll just add random intercepts to start, but will convert random slopes later.
...
df <- sim_design(
  between = list(version = c("easy", "hard"),
...
get_coefs(data = df, formula = y ~ vers * cond)
```
...
```{r sim-lmer-default}
sim_lmer(cond_eff = 100, error_sd = 250)
```
