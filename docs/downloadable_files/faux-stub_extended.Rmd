...
```{r}
des <- get_design(dat_long)

dat_wide <- long2wide(dat_long)
```
...
analyse <- function(data) {
  t.test(data$W1a, data$W1b, paired = TRUE) |> 
    broom::tidy()
}
...
analyse <- function(data) {
  t.test(y ~ pet, data = data) |>
    broom::tidy()
}

sim_n <- function(n, rep = 1000) {
  dat <- sim_design(
    n = n,
    between = list(pet = c("cat", "dog")),
    mu = c(0, 0.5), 
...
```{r}
mu <- apply(attitude, 2, mean)
sd <- apply(attitude, 2, sd)
r <- cor(attitude)

rnorm_multi(n = 20, mu = mu, sd = sd, r = r)
```
...
df <- sim_design(
  ...
  within = list(time = c("pre", "post")),
  n = 10,
  mu = c(10, 12, 14, 16, 18, 20),
  sd = 5,
  r = list(cat = .25, dog = 0, ferret = -.25)
)
...
```{r}
x <- rnorm(20)
y <- rnorm_pre(x, mu = 10, sd = 2, r = 0.9, empirical = F)
...
```{r}
rnorm_multi(n = 10, vars = 3) |>
  messy(prop = 0.1, "X1", "X2") |>
  messy(prop = 0.5, "X3", replace = 999)
```
